...
using LinearAlgebra, MinimallyDisruptiveCurves, Plots, OrdinaryDiffEq, ModelingToolkit, JuMP, Ipopt, PlutoUI
...
md"""
# Circadian Oscillator / "injection loss" example


- Here we model a big circadian oscillator system from https://www.pnas.org/content/pnas/suppl/2003/05/27/1132112100.DC1/2112SuppText.pdf

- Accompanying article is here https://www.pnas.org/content/100/12/7051

### Why?

- It's a big ODE system. Running a minimally disruptive curve on the trajectory of the system (which involves computing the trajectory and it's gradient, as a function of parameters), can take a while in principle.

### Alternative?

Yes! Read on...

Suppose we want to find minimally disruptive curves preserving model behaviour from a nominal trajectory: $x^*(t)$, which has a differential equation:

$\dot{x}^*(t) = f(x(t), p^*)$

(Here $p^*$ are the fitted parameters we want to explore)

**Previously** minimally disruptive curves have used as their cost function the difference between the nominal and parameterised trajectory. In otherwords, they find trajectories in parameter space where 

$$\|x^*(t) - x(t,p)\|_2^2$$ is small. (or some other distance metric apart from the L2 norm)

---
**In this notebook** we do something different (and more computationally efficient):

The "injection_loss"

```julia
build_injection_loss
```

Instead of defining the cost as the mean square error between trajectories, we can measure it as the mean square error between **derivatives of the ODE's vector field along the nominal trajectory**. In other words, given a parameter vector $p$, at each timepoint we:

1. Access the state of the **nominal** ODE, $x^*(t)$
2. Calculate $$\text{Cost}(p) = \int^T_0 \| f(x^*(t), p^*) - f(x^*(t), p) \|_2^2$$ \ dt

(This is often known as a collocation cost). Note that we **don't have to solve an ODE** to calculate the cost function or its gradient. Which makes this cost function really efficient to calculate. 

I called it injection loss as it's the amount of input to 'inject' into the vector field to make it match the nominal vector field $f(x(t), p^*)$. For neuron models this literally corresponds to a current injection. 

### Collocation/injection loss is bad at fitting

This loss isn't usually used for optimisation, i.e. fitting. It's fragile to noise and hitting local minima .

### Collocation/injection loss is good at MDC-ing!
...
# v_sP is the transcription rate of PER mRNA. In the paper, the effect of light is modelled through changes in this parameter.
...
[ D(M_P) ~ v_sP * (B_N^n / (K_AP^n + B_N^n)) - v_mP * (M_P / (K_mP + M_P)) - k_dmp * M_P,
  ...
  D(I_N) ~ -k_8 * I_N + k_7 * B_N * PC_N - v_dIN * I_N / (K_d + I_N) - k_dn * I_N
    ...
    od = ODESystem(eqs, t; name=:Circadian_Oscillator, defaults = Dict(ic..., ps...))
# of = ODEFunction(od)
    tspan = (0., 100.)
    return od
end

# ╔═╡ 7bf5f035-79f4-4b79-8295-61dcb5d86f6f
tspan = (0., 100.)

# ╔═╡ 0fae26a5-2f1e-4f5c-be8b-c1af61f2ba32
od = CircadianOscillator();

# ╔═╡ 7ebae7ad-f9ce-4ed5-a76f-ee1c0d7da33b
ModelingToolkit.get_defaults(od)

# ╔═╡ 6955c5c5-18c4-419c-a9e7-fb1ecf155c0e
prob = ODEProblem(od,[],tspan,[]);

# ╔═╡ 01747596-a4d0-4185-8e5b-698c1b8d985a
nom_sol = solve(prob, alg())

# ╔═╡ 4a296873-c658-4761-860d-0cc610e4a7e4
tsteps=nom_sol.t

# ╔═╡ 7c4190b8-3af4-4bb0-a376-d31004891f02
...
# ╔═╡ d07ec526-e64c-485e-a24c-fffcf613c772
lossf(sol, nom_sol) = sum( integrand(el1,el2) for (el1,el2) in zip(eachcol(sol), eachcol(nom_sol)  )  )

# ╔═╡ 66848237-0577-4835-b266-1aaae4779322
lossf1(sol) = lossf(sol, nom_sol);

# ╔═╡ 7a74091d-8216-4e08-ae9a-42305a5b25db
plot(nom_sol)

# ╔═╡ 1060299d-611b-4260-be55-a0f0b717cbcb
...
# ╔═╡ 42d32ad4-de87-4d4f-8039-00af312189e5
md""" 
Select here whether you would like to go in the positive or negative direction

(In a normal Julia REPL, you can do both by taking the span as $(a, b)$ where $a<0$ and $b>0$). In this case, MinimallyDisruptiveCurves runs two curves on $(a, 0)$ and $(0, b)$, on separate threads, before merging them into a single solution. **Unfortunately** on Pluto.jl notebooks this multithreading solution hangs. 

`direction = ` $(@bind pm html"<select><option value=+>+</option><option value=->-</option></select>")
"""

# ╔═╡ 7e4d01a7-7727-4c10-9ef1-c7fe75b788ab
eval(Symbol(pm))
...
begin
    p1 = plot(mdc; pnames = parameters(od), legend = :bottom)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "db905b90547d3dfccb05eee556a1e7ca1ba6d8b5"
